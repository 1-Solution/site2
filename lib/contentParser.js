...
export const getSinglePage = (folder) => {
  ...
  const singlePages = filterSingleFiles.map((filename) => {
    ...
    const locale = filename.replace(".md", "").split(".")[1] ?? false;
    ...
    return { frontmatter: frontmatter, slug: url, content: content, locale };
  });
...
export const getRegularPage = async (slug, locale) => {
  const publishedPages = getSinglePage("content");
  const pageData = publishedPages.filter((data) => data.slug === slug && data.locale === locale);
  const notFoundPage = fs.readFileSync(path.join("content/404.md"), "utf-8");
  const notFoundDataParsed = matter(notFoundPage);
